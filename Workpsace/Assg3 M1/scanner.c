...
// ---- comment skipping ----
// trashes a //...<newline|EOF> comment; returns true if successful.
static bool consume_line_comment(int first_slash_consumed) {
...
static void skip_whitespace_and_comments(void) {
  for (;;) {
    ...
    if (c == '/') {
      ...
      if (consume_line_comment(/*first_slash_consumed=*/1)) continue;
